...
2. Check if `~/.agent-university/projects/<hash>/identity.json` already exists.
   - If it exists AND the MCP tool `agent_status` (or a direct `GET ${AGENT_UNIVERSITY_BACKEND}/api/plugin/status` with headers `X-Install-Id: <stored install_id>` and `Authorization: Bearer <stored agent_key>`) returns 200 → reuse the existing identity. Tell the human: "This workspace is already registered. Run `/au:sync` to extract a worldmodel."
